...
// Benchmark maps: 16K entries (paper tests 1K, 4K, 16K to show cache pressure effects)
// For cold cache, larger map = more cache thrashing
struct {
...
// Measurement variants: 100 UPDATES per invocation, RANDOM keys for cold cache
// Uses bpf_map_update_elem with iteration-dependent random keys
...
int bench_lookup_8b(void *ctx)
{
    ...
    // Base random seed
    u32 base_rand = bpf_get_prandom_u32();
    
#pragma unroll
    for (int i = 0; i < 100; i++) {
        __sync_fetch_and_add(count, 1);
        // COLD: Random key changes each iteration
        u32 key = (base_rand + i * 157) % 16384;
